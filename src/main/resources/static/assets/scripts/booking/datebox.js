...
document.addEventListener('DOMContentLoaded', function () {
  ...
  let currentDate = new Date('2024-05-01'); // 현재 날짜를 설정
  let isAnimating = false; // 애니메이션 진행 중인지를 추적
  let selectedDate = new Date('2024-05-01'); // Store the selected date
  ...
  function clickTodayButton() {
    ...
    const todayDateString = `${todayFormatted.year}-${todayFormatted.month}-${todayFormatted.day}`;
    const todayButton = document.querySelector(`.date-area .wrap .date[data-date="${todayDateString}"]`);
    if (todayButton) {
      todayButton.click();
    }
  }
  ...
  // Expose getSelectedDate globally
